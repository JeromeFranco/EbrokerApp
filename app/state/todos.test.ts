...
it('should add todo', () => {
  ...
  const expected: TodosState = { ...initialState, items: [...initialState.items, todo] };
  expect(todosReducer(undefined, todosActions.addTodo(todo.title))).toEqual(expected);
});

it('should remove todo', () => {
  const expected: TodosState = { ...initialState, items: initialState.items.filter(item => item.id !== 1) };
